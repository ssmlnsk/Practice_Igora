...
class Database:
    ...
    def __init__(self):
        ...
        """
        self.conn = mysql.connector.connect(host='192.168.0.35', port=3306, user='user15', password="72925", database='user15')
    ...
    def update_req(self, id, code, date, time, client, service, status, date2, rent, emp):
        """
        Обновление заказов
        :return: None
        """
        cursor = self.conn.cursor()
        if date2 == 'None':
            cursor.execute(f"UPDATE request set Code='{code}', DateOfCreate='{date}', TimeOfCreate='{time}', Client='{client}', Service='{service}', Status='{status}', RentalTime='{rent}', Employee='{emp}' WHERE ID='{id}'")
        else:
            cursor.execute(f"UPDATE request set Code='{code}', DateOfCreate='{date}', TimeOfCreate='{time}', Client='{client}', Service='{service}', Status='{status}', DateOfCancel='{date2}', RentalTime='{rent}', Employee='{emp}' WHERE ID='{id}'")
        self.conn.commit()
    ...
    def get_req_serv(self):
        cursor = self.conn.cursor()
        cursor.execute(f"SELECT Code, COUNT(Service) as serv FROM request GROUP BY Code")
        rows = cursor.fetchall()
        return rows

    def get_req_count(self):
        cursor = self.conn.cursor()
        cursor.execute(f"SELECT DateOfCreate, COUNT(DISTINCT(Code)) as req FROM request GROUP BY DateOfCreate")
        rows = cursor.fetchall()
        return rows
