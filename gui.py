...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.btn_all_req.clicked.connect(self.page_all_req)
        ...
        self.ui.btn_delete_req.clicked.connect(self.delete_req)
        self.ui.btn_save_req.clicked.connect(self.save_req)
    ...
    def exit(self, block):
        ...
        self.page.setCurrentIndex(self.page_id[self.now_page])
        t = time.localtime()
        time_string = time.strftime("%Y:%m:%d %H:%M:%S", t)  # время выхода
    ...
    def updateTableReq(self):
        """
        Отвечает за обновление таблицы заказов
        :return:
        """
        self.table_req.clear()
        rec = self.facade.read_requests()
        self.ui.table_req.setColumnCount(10)
        self.ui.table_req.setRowCount(len(rec))
        self.ui.table_req.setHorizontalHeaderLabels(['ID', 'Код заказа', 'Дата создания', 'Время создания', 'Клиент', 'Услуга', 'Статус', 'Дата закрытия', 'Время проката', 'Сотрудник'])

        for i, request in enumerate(rec):
            for x, field in enumerate(request):
                item = QTableWidgetItem()
                item.setText(str(field))
                if x == 0:
                    item.setFlags(Qt.ItemIsEnabled)
                self.ui.table_req.setItem(i, x, item)
    ...
    def delete_req(self):
        ...
        :return:
        """
        SelectedRow = self.table_req.currentRow()
        rowcount = self.table_req.rowCount()
        colcount = self.table_req.columnCount()
        ...
        if rowcount == 0:
            ...
            for col in range(1, colcount):
                self.table_req.setItem(SelectedRow, col, QTableWidgetItem(''))
            ix = self.table_req.model().index(-1, -1)
            self.table_req.setCurrentIndex(ix)
    ...
    def get_from_table_req(self):
        ...
        :return: data
        """
        rows = self.table_req.rowCount()
        cols = self.table_req.columnCount()
        data = []
        for row in range(rows):
            tmp = []
            for col in range(cols):
                tmp.append(self.table_req.item(row, col).text())
            data.append(tmp)
        return data
    ...
    def save_req(self):
        ...
        for string in data:
            if string[1] != '':  # если название услуги есть, то обновляем данные
                self.facade.update_req(int(string[0]), string[1], string[2], string[3], string[4], string[5], string[6], string[7], string[8], string[9])
            else:  # если названия услуги нет, то удаляем эту строку
    ...
    def add_new_request(self):
        ...
        cl1 = self.comboBox_clients.currentText().split()
        cl2 = self.facade.get_id_client(cl1[0], cl1[1], cl1[2])
        self.client_code = QListWidgetItem(cl2)
        ...
        self.date_req = str(self.datetime.strftime("%Y.%m.%d"))
    ...
    def save_request(self):
        ...
        for a in serv:
            if a != 'o':
                self.facade.create_request(number, request[1], request[2], request[3], self.now_login, a)
            else:
                continue
    ...
    def otchot(self):
        ...
        data = self.facade.get_req_count()
        count_serv = {}  # 1
        count_order_serv = {}  # 2
        count_order = {}  # 3
        ...
        for i, date in enumerate(data):
            data[i] = list(data[i])
            d = str(date[0]).split('-')
            data[i][0] = d[0] + '-' + d[1] + '-' + d[2]
            if start <= data[i][0] <= end:
                count_order[str(date[0])] = date[1]
                # try:
                #     count_order[str(date[1])] = data_info_req
                # except KeyError:
                #     count_order[str(date[1])] = data_info_req
                # try:
                #     count_serv[str(date[1])] = data_info_serv
                # except:
                #     count_serv[str(date[1])] = data_info_serv
                # try:
...
class DialogAuth(QDialog):
    ...
    def enter(self):
        ...
        if auth_log == '' or auth_pas == '':
            ...
            password, role, last_exit, block, fio, photo, id = self.parent().facade.get_for_authorization(auth_log)
            ...
            if last_exit is not None and block:     # после окончания предыдущей сессии, новую можно начать только через 3 минуты
                last_exit = str(last_exit).split()
                year, mon, day = map(int, last_exit[0].split('-'))
            ...
            if self.now_captcha is not None and self.now_captcha != self.ui.edit_captcha.text():
                ...
                if password != '':  # если нет пароля, значит нет пользователя с введенным логином, поэтому записывать в историю входа не надо
                    ...
                    self.parent().facade.insert_time_entry(id, time_entry, False)
                ...
                self.parent().facade.insert_time_entry(id, time_entry, True)
...
class DialogNewClient(QDialog):
    ...
    def add(self):
        ...
        if self.surname != '' and self.name != '' and self.lastname != '' and self.passportData != '' and self.dateOfBirth != '' and self.address != '' and self.email != '':
            self.facade.insert_client(self.surname, self.name, self.lastname, self.passportData, self.dateOfBirth, self.address, self.email)
        else:
            self.mes_box('Заполните все поля!')
